...
namespace Helpline_MVC_Project.BLL
{
    public class BusinessLayer
    {
        ...
        public string UnreadGroupList(string uid, out bool success)
        {
            success = false;
            string UnreadMsg = "";
            SqlConnection con = null;
            ...
            {
                con = new SqlConnection(conn);
                SqlCommand cmd = new SqlCommand("usp_Helpline_GetUnreadGroupMsg", con);
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        ...
                        string gid = dr[2].ToString();
                        UnreadMsg += "<span class='notify'><a href='/Home/Groups?gid=" + gid + "'>You have " + count + " unread message(s) from " + groupName + "</a><br/>";
                    }
                }
                con.Close();
                success = true;
        ...
        public void ReadGroupMsg(int uid, int gid, out bool success)
        {
            success = false;
            string lblPrevMsg = string.Empty;
            SqlConnection con = null;
            ...
            {
                con = new SqlConnection(conn);
                SqlCommand cmd = new SqlCommand("usp_Helpline_ReadGroupMsg", con);
                cmd.CommandType = CommandType.StoredProcedure;
